...
class MCPJungleClient:
    ...
    def register_server(
        self, server_config: dict[str, Any], *, timeout: int | None = None
    ) -> str:
        ...
        try:
            result = self._run(["register", "--conf", config_path], timeout=timeout)
        finally:
    ...
    def list_tools_raw(self) -> str:
        """List all tools across all servers (raw text output)."""
        return self._run(["list", "tools"]).stdout
